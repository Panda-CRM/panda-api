...
func ValidateCPF(cpf string) []error {
	...
	if DEBBUG == true { fmt.Println("CPF: ", cpf) }
	...
	if cpf != "" {
		...
		if DEBBUG == true { fmt.Println("Removido pontos: ", cpfValidate) }
		...
		if len(cpfValidate) == 11 {
			...
			if 	(num1 	== num2)	&& 
				...
				if DEBBUG == true { fmt.Println("Soma1: ", sum1) }

				// Faz a divisão da soma por 11 para pegar o resto
				rest1 := (sum1 * 10) % 11
				if DEBBUG == true { fmt.Println("Resto1: ", rest1) }
				...
				if DEBBUG == true { fmt.Println("Soma2: ", sum2) }

				// Faz a divisão da soma por 11 para pegar o resto
				rest2 := (sum2 * 10) % 11
				if DEBBUG == true { fmt.Println("Resto2: ", rest2) }
				...
				if (rest1 == num10) && (rest2 == num11) {
					if DEBBUG == true { fmt.Println("Passou na validação!") }
					return nil
				} else {
					errors = append(errors, ErrInvalidCPF)
				}
...
func ValidateCNPJ(cnpj string) []error {
	...
	if DEBBUG == true { fmt.Println("CNPJ: ", cnpj) }
	...
	if cnpj != "" {
		...
		if DEBBUG == true { fmt.Println("Removido pontos: ", cnpjValidate) }
...
func Test_ValidateCNPJ() {
	...
	// Bateria de Testes
	var cnpjs [10]string
	cnpjs[0] = "12345678000195"
	cnpjs[1] = "12.345.678/0001-95"
	cnpjs[2] = "11111111111111"
	cnpjs[3] = "11.111.111/1111-11"
	cnpjs[4] = "123456780001950"
	cnpjs[5] = "12.345.678/0001-950"
	cnpjs[6] = "12345678912345X"
	cnpjs[7] = "12.345.678/9123-45X"
	cnpjs[8] = "XXXXXXXXXXXXXX"
	cnpjs[9] = "XX.XXX.XXX/XXXX-XX"

	for _, cnpj := range cnpjs {
		fmt.Println("---- Teste Iniciado ----")
		if DEBBUG == false { fmt.Println("CNPJ: ", cnpj) }
		fmt.Println("Erros:", ValidateCNPJ(cnpj))
		fmt.Println("------------------------")
		fmt.Println()
	}
}

func Test_ValidateCPF() {
	...
	var cpfs [9]string
	cpfs[0] = "41678171875"
	cpfs[1] = "416.781.718-75"
	cpfs[2] = "22222222222"
	cpfs[3] = "222.222.222-22"
	cpfs[4] = "XXX.XXX.XXX-XX"
	cpfs[5] = "XXXXXXXXXXX"
	cpfs[6] = "416.X81.718-75"
	cpfs[7] = ""
	cpfs[8] = "123456789123456"

	for _, cpf := range cpfs {
		fmt.Println("---- Teste Iniciado ----")
		if DEBBUG == false { fmt.Println("CPF: ", cpf) }
		fmt.Println("Erros:", ValidateCPF(cpf))
		fmt.Println("------------------------")
		fmt.Println()
	}
}